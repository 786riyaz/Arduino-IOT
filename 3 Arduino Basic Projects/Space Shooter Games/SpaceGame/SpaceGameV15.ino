...
void spawnEnemy() {
  ...
  for (int i = 0; i < MAX_ENEMIES; i++) {
    if (!enemies[i].active) {
      ...
      Serial.print(F("Spawned enemy type: ")); Serial.print(enemies[i].type);
      Serial.print(F(" at x: ")); Serial.println(enemies[i].x);
      break;
    }
  }
}
  ...
  do {
    if (!shieldActive || (millis() % 200 < 100)) {
      if (playerX <= 121) {
        ...
        Serial.print(F("Drawing player at x: ")); Serial.println(playerX);
      } else {
        Serial.print(F("Player clipped at x: ")); Serial.println(playerX);
      }
    }
    for (int i = 0; i < MAX_BULLETS; i++) {
    ...
    for (int i = 0; i < MAX_ENEMIES; i++) {
      if (enemies[i].active && enemies[i].x <= 121) {
        if (enemies[i].type == COLLIDER) {
          ...
          Serial.print(F("Drawing Collider at x: ")); Serial.println(enemies[i].x);
        } else if (enemies[i].type == BULLET) {
          u8g2.drawXBMP(enemies[i].x, enemies[i].y, 7, 7, bulletEnemyBitmap);
          Serial.print(F("Drawing Bullet at x: ")); Serial.println(enemies[i].x);
        } else if (enemies[i].type == LASER) {
          u8g2.drawXBMP(enemies[i].x, enemies[i].y, 7, 7, laserEnemyBitmap);
          Serial.print(F("Drawing Laser at x: ")); Serial.println(enemies[i].x);
        }
      } else if (enemies[i].active) {
        Serial.print(F("Enemy clipped at x: ")); Serial.println(enemies[i].x);
      }
    }
    for (int i = 0; i < MAX_POWERUPS; i++) {
