...
enum EnemyType { E_COLLIDER,
                 E_BULLET,
                 E_LASER };
enum PowerUpType { P_NONE,
                   P_EXTRA_LIFE,
                   P_SHIELD,
                   P_BIG_BANG };
...
void drawHeart(int16_t x, int16_t y) {
  ...
  tft.drawPixel(x + 2, y + 1, COL_HP);
  tft.drawPixel(x + 3, y + 1, COL_HP);
  tft.drawPixel(x + 4, y + 1, COL_HP);
  tft.drawPixel(x + 1, y + 2, COL_HP);
  tft.drawPixel(x + 2, y + 2, COL_HP);
  tft.drawPixel(x + 3, y + 2, COL_HP);
  tft.drawPixel(x + 4, y + 2, COL_HP);
  tft.drawPixel(x + 5, y + 2, COL_HP);
  tft.drawPixel(x + 1, y + 3, COL_HP);
  tft.drawPixel(x + 2, y + 3, COL_HP);
  tft.drawPixel(x + 3, y + 3, COL_HP);
  tft.drawPixel(x + 4, y + 3, COL_HP);
  tft.drawPixel(x + 5, y + 3, COL_HP);
  tft.drawPixel(x + 2, y + 4, COL_HP);
  tft.drawPixel(x + 3, y + 4, COL_HP);
  tft.drawPixel(x + 4, y + 4, COL_HP);
...
void startGame() {
  ...
  for (uint8_t j = 0; j < MAX_ENEMIES && spawned < 5; j++) {
    if (!enemies[j].active) {
      ...
      enemies[j].px = enemies[j].x;
      enemies[j].py = enemies[j].y;
      enemies[j].active = true;
      uint8_t r = random(100);
      enemies[j].type = (r < 40) ? E_COLLIDER : (r < 70) ? E_BULLET
                                                         : E_LASER;
      spawned++;
    }
  }
...
void spawnEnemy() {
  ...
  for (uint8_t i = 0; i < MAX_ENEMIES; i++) {
    if (!enemies[i].active) {
      ...
      uint8_t r = random(100);
      enemies[i].type = (r < 40) ? E_COLLIDER : (r < 70) ? E_BULLET
                                                         : E_LASER;
      drawEnemy(enemies[i]);
      break;
    }
...
void spawnEnemyLaser(int16_t x, int16_t y) {
  for (uint8_t i = 0; i < MAX_ENEMY_LASERS; i++) {
    if (!enemyLasers[i].active) {
      ...
      enemyLasers[i].h = 10;
      enemyLasers[i].x = x + 4;
      enemyLasers[i].y = y + 10;
      enemyLasers[i].px = enemyLasers[i].x;
      enemyLasers[i].py = enemyLasers[i].y;
...
void spawnPowerUp() {
  for (uint8_t i = 0; i < MAX_POWERUPS; i++) {
    if (!powerups[i].active) {
      ...
      powerups[i].y = 14;
      powerups[i].px = powerups[i].x;
      powerups[i].py = powerups[i].y;
      powerups[i].active = true;
      uint8_t r = random(100);
      powerups[i].type = (r < 20) ? P_BIG_BANG : (r < 50) ? P_EXTRA_LIFE
                                                          : P_SHIELD;
...
inline bool btnP() {
  return digitalRead(BTN_PAUSE) == LOW;
}
inline bool joyBtn() {
  return digitalRead(JOY_SW_PIN) == LOW;
}

// FIXED: Invert X-axis to correct joystick movement (right moves right, left moves left)
int16_t getJoyX() {
  int16_t raw = analogRead(JOY_X_PIN);
  int16_t centered = -(raw - joyXCenter);  // Invert the X-axis by negating the centered value
...
void loop() {
  ...
  for (uint8_t i = 0; i < MAX_ENEMIES; i++) {
    ...
    if (playerVulnerable && checkCollision(playerX, playerY, playerW, playerH, enemies[i].x, enemies[i].y, enemies[i].w, enemies[i].h)) {
      eraseRect(enemies[i].x, enemies[i].y, enemies[i].w, enemies[i].h);
      enemies[i].active = false;
      lives--;
      playDamageSound();
  ...
  for (uint8_t i = 0; i < MAX_ENEMY_BULLETS; i++) {
    ...
    if (playerVulnerable && checkCollision(playerX, playerY, playerW, playerH, enemyBullets[i].x, enemyBullets[i].y, enemyBullets[i].w, enemyBullets[i].h)) {
      eraseRect(enemyBullets[i].x, enemyBullets[i].y, enemyBullets[i].w, enemyBullets[i].h);
      enemyBullets[i].active = false;
      lives--;
      playDamageSound();
  ...
  for (uint8_t i = 0; i < MAX_ENEMY_LASERS; i++) {
    ...
    if (playerVulnerable && checkCollision(playerX, playerY, playerW, playerH, enemyLasers[i].x, enemyLasers[i].y, enemyLasers[i].w, enemyLasers[i].h)) {
      eraseRect(enemyLasers[i].x, enemyLasers[i].y, enemyLasers[i].w, enemyLasers[i].h);
      enemyLasers[i].active = false;
      lives--;
      playDamageSound();
