...
unsigned char lastSecond = 0; // Track previous second for minute transition

// Buzzer and button pins
#define BUZZER_PIN 13
#define BUTTON_DECREASE_PIN 3 // Button for decreasing timer interval
#define BUTTON_INCREASE_PIN 2 // Button for increasing timer interval
#define BUTTON_HOURLY_BEEP_PIN 6 // Slider for hourly beep
...
// Array of intervals in milliseconds: 1 hour, 30 min, 15 min, 10 min, 5 min, 1 min, no beep
const unsigned long intervals[] = {3600000, 1800000, 900000, 600000, 300000, 60000, ULONG_MAX};
const int numIntervals = 7; // Number of intervals
int currentIntervalIndex = 6; // Start with no beep (index 6)
...
void setup() {
  ...
  if (Wire.endTransmission() == 0) {
    ...
    if (rtc.lostPower()) {
      ...
      // Set to current time (e.g., August 26, 2025, 12:24:00 AM IST)
      rtc.adjust(DateTime(2025, 8, 26, 1 6, 5, 0));
    }
...
void loop() {
  ...
  // Check if the selected interval has passed (skip if "no beep" mode)
  ...
  if (anyBeepEnabled && second == 0 && lastSecond == 59) {
    ...
    if (beepEnabled[0] && minute == 0) { // Hourly beep
      shouldBeep = true;
      Serial.println(F("Hourly beep triggered"));
    }
    if (beepEnabled[1] && (minute == 0 || minute == 30)) { // 30-min beep
      shouldBeep = true;
      Serial.println(F("30-min beep triggered"));
    }
    if (beepEnabled[2] && (minute % 15 == 0)) { // 15-min beep
      shouldBeep = true;
      Serial.println(F("15-min beep triggered"));
    }
    if (beepEnabled[3] && (minute % 10 == 0)) { // 10-min beep
      shouldBeep = true;
      Serial.println(F("10-min beep triggered"));
    }
    if (beepEnabled[4] && (minute % 5 == 0)) { // 5-min beep
      shouldBeep = true;
      Serial.println(F("5-min beep triggered"));
    }
    if (beepEnabled[5]) { // 1-min beep
      shouldBeep = true;
  ...
  // Calculate remaining time until next interval beep (show 00:00:00 for "no beep" mode)
  ...
  // Format remaining time (e.g., "Beep: 01:00:00" or "Beep: 00:00:00" for no beep)
  char timerBuf[16];
