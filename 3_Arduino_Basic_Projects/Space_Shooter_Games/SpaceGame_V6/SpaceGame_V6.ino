...
// ---------- GAME LOOP HELPERS ----------
void updateUIHeader(){
  // IMPROVED: New layout - Lives (left), High Score (center), Current Score (right)
  ...
  // Lives at top left (hearts)
  drawLives();
  
  // High Score at top center
  tft.setCursor(SCREEN_W/2 - 18, 2); // Approximately center
  tft.print("HI:");
  tft.print(highScore);
  
  // Current Score at top right
  tft.setCursor(SCREEN_W - 35, 2); // Right aligned
  tft.print("S:");
  tft.print(score);
}
...
void loop(){
  ...
  // FIRE with improved rate limiting
